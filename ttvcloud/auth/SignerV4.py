...
class SignerV4(object):
    ...
    def sign(request, credentials):
        ...
        request.headers['X-Amz-Date'] = format_date

        md = MetaData()
        md.set_algorithm('AWS4-HMAC-SHA256')
        ...
        md.set_credential_scope('/'.join([md.date, md.region, md.service, 'aws4_request']))
    ...
    def sign_url(request, credentials):
        ...
        query = request.query
        query['X-Amz-Date'] = format_date
        query['X-Amz-NotSignBody'] = ''
        query['X-Amz-Credential'] = credentials.ak + '/' + md.credential_scope
        query['X-Amz-Algorithm'] = md.algorithm
        query['X-Amz-SignedHeaders'] = md.signed_headers
        query['X-Amz-SignedQueries'] = ''

        query['X-Amz-SignedQueries'] = ';'.join(sorted(query.keys()))
